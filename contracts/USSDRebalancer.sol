...
contract USSDRebalancer is AccessControlUpgradeable, IUSSDRebalancer {
    ...
    function getSupplyProportion() public view returns (uint256, uint256) {// volume of ussd/dai
    ...
    function BuyUSSDSellCollateral(uint256 amountToBuy) internal {
        ...
        for (uint256 i = 0; i < collateral.length; i++) {
            ...
            if (collateralval > amountToBuyLeftUSD) {
                ...
                if (collateralval >= amountToBuyLeftUSD / 20) {
                    ...
                    IUSSD(USSD)
                        .UniV3SwapInput(collateral[i].pathsell, IERC20Upgradeable(collateral[i].token).balanceOf(USSD));
        ...
        if (DAItosell > 0) {
            if (uniPool.token0() == USSD) {
                IUSSD(USSD)
                    .UniV3SwapInput(
                        bytes.concat(abi.encodePacked(uniPool.token1(), hex"0001f4", uniPool.token0())), DAItosell
                    );
            } else {
                IUSSD(USSD)
                    .UniV3SwapInput(
                        bytes.concat(abi.encodePacked(uniPool.token0(), hex"0001f4", uniPool.token1())), DAItosell
                    );
            }
        }
    ...
    function SellUSSDBuyCollateral() internal {
        ...
        if (uniPool.token0() == USSD) {
            ...
            daibought = IERC20Upgradeable(baseAsset).balanceOf(USSD);
            IUSSD(USSD)
                .UniV3SwapInput(bytes.concat(abi.encodePacked(uniPool.token1(), hex"0001f4", uniPool.token0())), amount);
            daibought = IERC20Upgradeable(baseAsset).balanceOf(USSD) - daibought; // would revert if not bought
        }
