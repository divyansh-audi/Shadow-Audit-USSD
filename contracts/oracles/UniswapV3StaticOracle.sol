...
/// @notice this file is taken from https://github.com/Mean-Finance/uniswap-v3-oracle/blob/main/solidity/contracts/StaticOracle.sol
...
contract StaticOracle is IStaticOracle {
    ...
    function isPairSupported(address _tokenA, address _tokenB) external view override returns (bool) {
        ...
        for (uint256 i; i < _length; ++i) {
            address _pool = PoolAddress.computeAddress(
                address(UNISWAP_V3_FACTORY), PoolAddress.getPoolKey(_tokenA, _tokenB, _knownFeeTiers[i])
            );
    ...
    /// @inheritdoc IStaticOracle
    function prepareAllAvailablePoolsWithTimePeriod(address _tokenA, address _tokenB, uint32 _period)
        external
        override
        returns (address[] memory _preparedPools)
    {
    ...
    function prepareSpecificFeeTiersWithTimePeriod(
        ...
        return prepareSpecificFeeTiersWithCardinality(
            _tokenA, _tokenB, _feeTiers, _getCardinalityForTimePeriod(_period)
        );
    }
    ...
    /// @inheritdoc IStaticOracle
    function prepareAllAvailablePoolsWithCardinality(address _tokenA, address _tokenB, uint16 _cardinality)
        public
        override
        returns (address[] memory _preparedPools)
    {
    ...
    function _getQueryablePoolsForTiers(address _tokenA, address _tokenB, uint32 _period)
        internal
        view
        virtual
        returns (address[] memory _queryablePools)
    {
    ...
    function _getPoolsForTiers(address _tokenA, address _tokenB, uint24[] memory _feeTiers)
        internal
        view
        virtual
        returns (address[] memory _pools)
    {
        ...
        for (uint256 i; i < _feeTiers.length; i++) {
            address _pool = PoolAddress.computeAddress(
                address(UNISWAP_V3_FACTORY), PoolAddress.getPoolKey(_tokenA, _tokenB, _feeTiers[i])
            );
            if (Address.isContract(_pool)) {
